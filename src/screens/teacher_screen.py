...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("You Don't Have Any Subjects Created Yet! Create One For Proceedings.")
        return
    ...
    with col1:
        selected_subject_label = st.selectbox('Select Subject', options=list(subject_options.keys()), width="stretch")
    with col2:
    ...
    with c1:
        if st.button("Delete All Photos", width='stretch', type='secondary', icon=":material/delete:", disabled=not has_photos):
            st.session_state.attendance_images = []
            st.rerun()

    with c2: 
        if st.button("Face Attendance", width='stretch', type='primary', icon=":material/ar_on_you:", disabled=not has_photos):
    ...
    with c3:
        if st.button("Voice Attendance", width='stretch', type='tertiary', icon=":material/frame_person_mic:"):
...
def teacher_tab_manage_subjects():
    ...
    with col1:
        st.header("Manage Students Subjects")
    with col2:
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter Username:", placeholder='Niteshghar')
    ...
    with btnc2:
        if st.button("Register Instead", type="primary", icon=':material/how_to_reg:', width="stretch"):
...
def register_teacher(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm):
    if not teacher_username or not teacher_name or not teacher_pass:
        return False, "All Fields Need To Be Filled!"
    ...
    try:
        ...
        return True, "Registered Successfully! Kindly Login Now."
    except Exception as e:
        return False, "Error Occurred While Registering Teacher!"
...
def teacher_screen_register():
    ...
    with c2:
        if st.button("Back To Home", type='primary', key='loginbackbtn', shortcut="control+backspace"):
            st.session_state['login_type'] = None
            st.rerun()
    ...
    teacher_username = st.text_input("Enter Username:", placeholder='@nitesh.ghar')
    teacher_name = st.text_input("Enter Name:", placeholder='Nitesh Ghar')
    ...
    btnc1, btnc2 = st.columns(2, gap='large')
    with btnc1:
        if st.button("Register Now", icon=':material/how_to_reg:', shortcut="control + enter", width="stretch"):
