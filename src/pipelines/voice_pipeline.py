...
def identify_speaker(new_embedding, candidate_dict, threshold=0.65):
    ...
    for stud_id, stored_embedding in candidate_dict.items():
        if stored_embedding:
...
def process_bulk_audio(audio_bytes, candidate_dict, threshold=0.65):
    try:
        ...
        for start, end in segments:
            ...
            s_id, score = identify_speaker(embedding, candidate_dict, threshold)

            if s_id:
                if s_id not in identified_results or score > identified_results[s_id]:
                    identified_results[s_id] = score
